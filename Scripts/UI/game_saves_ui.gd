...
@export var load_new_saves : bool = false
...
func _setup():
	...
	if  new_save_button and not new_save_button.pressed.is_connected(_on_save_button_pressed):
		new_save_button.pressed.connect(_on_save_button_pressed)
	
	
	if  load_save_button and not load_save_button.pressed.is_connected(_on_load_save_button_pressed):
		new_save_button.pressed.connect(_on_load_save_button_pressed)
	
	if delete_save_button and not delete_save_button.pressed.is_connected(_on_save_button_pressed):
...
func _on_save_button_pressed() -> void:
	...
	GameManager.save_game_data(GameManager.get_current_scene_data(), save_name, load_new_saves)
