...
func instantiate_inventory_slot_ui(grid_coords : Vector2i):
	...
	if inventory_grid.grid_shape.get_grid_shape_cell(grid_coords.x, grid_coords.y, 0):
		instantiated_slot = GameManager.managers["InventoryManager"].inventory_slot_prefab.instantiate()
	else:
		instantiated_slot = GameManager.managers["InventoryManager"].inactive_inventory_slot_prefab.instantiate()
