...
signal action_execution_started(current_action_definition: BaseActionDefinition, grid_object : GridObject)

signal action_execution_finished(current_action_definition: BaseActionDefinition, grid_object : GridObject)
...
func _execute_action_internal(action_def: BaseActionDefinition, params: Dictionary) -> void:
	...
	action_execution_started.emit(action_def, params["unit"])
	...
	any_action_execution_finished.emit(action_def, params["unit"])
	action_execution_finished.emit(action_def, params["unit"])
	action_params.clear()
	set_is_busy(false,  null)
