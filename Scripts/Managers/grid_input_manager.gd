...
var visual : Node3D
@export var arrow_indicator_scene : PackedScene
...
func _process(_delta: float) -> void:
	...
	var new_grid_cell : GridCell = null
	if hit:
		# Check if the hit is on GRIDOBJECT layer but NOT on OBSTACLE layer
		if (hit.collider.collision_layer & PhysicsLayer.GRIDOBJECT):
			# Try to get grid cell from GridObject directly
			var current_node = hit.collider
			new_grid_cell = null
			while current_node:
				if current_node is GridObject and current_node.is_selectable:
					new_grid_cell = current_node.grid_position_data.grid_cell
					break
		...
		if new_grid_cell == null:
			# Fallback to standard grid cell detection (e.g. for obstacles or terrain)
			var hit_pos = hit.position
			var result = gridSystem.try_get_gridCell_from_world_position(hit_pos)
			if result["success"]:
				if result["grid_cell"].grid_cell_state == Enums.cellState.AIR:
					var temp_cell = gridSystem.get_cell_below_recursive(result["grid_cell"].grid_coordinates, Enums.cellState.GROUND)
					new_grid_cell = temp_cell
				else:
					new_grid_cell = result["grid_cell"]
			else:
				new_grid_cell = null
	else:
		new_grid_cell = null
	...
	for cell in new_path_dict.keys():
		...
		var arrow_scene : Node3D = arrow_indicator_scene.instantiate()
		var label_3d : Label3D = arrow_scene.get_node("Label3D")
		label_3d_holder.add_child(arrow_scene)
		...
		#label_3d.global_position = cell.world_position + Vector3(0, 1, 0)

		if next_cell_index < path.size():
			arrow_scene.global_position = cell.world_position + Vector3(0, 0.5, 0)
			var direction = (path[next_cell_index].world_position - cell.world_position).normalized()
			
			# Choose up vector based on direction to prevent colinearity
			var up_vector := Vector3.UP
			if abs(direction.dot(Vector3.UP)) > 0.999:
				up_vector = Vector3.FORWARD
	
			arrow_scene.look_at(arrow_scene.global_position + direction, up_vector)
		else:
