...
func commit_definition_to_globe_state(definition: HexCellDefinition) -> void:
	if definition == null:
		push_warning("No definition to commit.")
		return

	var globe_data: Dictionary = get_session_value("globe_state", {})
	...
	for i in range(definition_array.size()):
		...
		if (
			existing_cell_index == definition.cell_index
	...
	set_session_value("globe_state", globe_data)
