...
# Show ALL hex definitions (uses each definition's get_cell_color()).
@export var show_city_cells = true:
	set = set_show_city_cells
@export var city_cell_color = Color(0.9, 0.3, 0.3, 0.7) # kept for compatibility
@export var city_cell_highlight_elevation = 0.02

# Texture-based cell coloring (optional, heavy)
...
@export var assignment_chunk_size = 1000   # cities processed per frame
@export var cells_per_surface = 3000       # cells batched per surface
@export var yield_every_surfaces = 1       # yield after N surfaces
...
# City raw data parsing (threaded)
...
# Async rebuild state
var _city_rebuild_running = false
var _city_rebuild_pending = false
...
func set_show_city_cells(v: bool) -> void:
	show_city_cells = v
	if is_inside_tree():
		_rebuild_city_cells()
...
# -------- Optimized: draw all definitions with per-cell color, batched and yielding


func request_definitions_rebuild() -> void:
	if not show_city_cells:
		return
	# If a build is already running, mark pending and return
	if _city_rebuild_running:
		_city_rebuild_pending = true
		return
	# Kick off the (batched + yielding) rebuild
	_rebuild_city_cells()



func _rebuild_city_cells() -> void:
	if not show_city_cells or hex_grid_data == null or hex_grid_data.cell_definitions.is_empty():
		_city_cells_mesh.mesh = null
		return
	if _city_rebuild_running:
		_city_rebuild_pending = true
		return
	_city_rebuild_running = true
	call_deferred("_rebuild_city_cells_async")

func _rebuild_city_cells_async() -> void:
	...
	_city_rebuild_running = false
	if _city_rebuild_pending:
		_city_rebuild_pending = false
		_rebuild_city_cells()

# -------- Texture-driven coloring (optional)
