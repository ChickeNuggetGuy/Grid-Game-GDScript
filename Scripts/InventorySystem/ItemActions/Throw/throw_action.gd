...
func _init(parameters : Dictionary) -> void:
	...
	grid_cell_path = parameters["grid_cell_path"]
	vector3_path = parameters["vector3_path"]
...
func _execute() -> bool:
	...
	throw_visual.position =  vector3_path[0]
	for position in vector3_path:
...
func _action_complete():
	var end_grid_cell : GridCell = grid_cell_path[grid_cell_path.size() - 1]
