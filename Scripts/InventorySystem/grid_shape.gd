...
func load_data(data: Dictionary):
	...
	# Safety check to ensure we iterate a valid array
	...
	if loaded_grid.size() == expected_size:
		...
		#push_warning("GridShape: Size mismatch (Expected %s, Got %s). Attempting to repair..." % [expected_size, loaded_grid.size()])
