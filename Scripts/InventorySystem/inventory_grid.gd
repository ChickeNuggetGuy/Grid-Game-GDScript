...
@export var item_count: int:
	get:
		return try_get_item_array().size()
	set(_value):
		pass

signal inventory_changed()
signal item_added(item_added: ItemData)
signal item_removed(item_removed: ItemData, new_inventory: InventoryGrid)
...
func _rebuild_items_array() -> void:
	_items.clear()
	_items.resize(grid_shape.grid_depth)

	for y in range(grid_shape.grid_depth):
		_items[y] = []
...
func _duplicate() -> Resource:
	...
	new_grid.equipment_inventory = equipment_inventory
	...
	for item in try_get_item_array():
		if item == null:
			continue
		new_grid.try_place_item_at(
			item.duplicate(true),
			item.current_invenrtory_coords
		)
...
func initialize(data: Dictionary = {}) -> void:
	if not data.is_empty():
		...
		if grid_shape == null:
			push_error(
				"InventoryGrid: initialize() called with null grid_shape. "
				+ "Creating fallback GridShape.new()."
			)
			grid_shape = GridShape.new()
	...
	if grid_shape == null:
		push_error(
			"InventoryGrid: initialize() failed because grid_shape is null."
		)
		_items = []
		return

	if grid_shape.grid_width <= 0 or grid_shape.grid_depth <= 0:
		push_error(
			"InventoryGrid's grid_shape has invalid dimensions (%d,%d). "
			+ "Cannot initialize _items array." % [
				grid_shape.grid_width,
				grid_shape.grid_depth
			]
		)
		_items = []
		return

	_rebuild_items_array()
		...
		if items_value is Array:
			for item_data in items_value:
				...
				if result.get("success", false):
					try_place_item_at(
						result["inventory_item"].duplicate(true),
						coordinates
					)
				else:
					print("Getting Item Failed: ", item_name)
			...
			for item_name in items_value:
				...
				if coords_val is String:
					var parts = coords_val.trim_prefix("(").trim_suffix(
						")"
					).split(",")

					if parts.size() == 2:
						var x : int = parts[0].strip_edges().to_int()
						var y : int = parts[1].strip_edges().to_int()
						coordinates = Vector2i(x, y)
					else:
						push_error(
							"Could not parse coordinates from string for "
							+ "item '%s': %s" % [item_name, coords_val]
						)
						continue
				else:
					push_error(
						"Unexpected coordinate format for item '%s': %s" % [
							item_name,
							typeof(coords_val)
						]
					)
...
func can_place_item_at(item: ItemData, position: Vector2i) -> bool:
	if item == null or item.grid_shape == null or position.x < 0 or position.y < 0:
		printerr(
			"InventoryGrid: Either item is null, item grid shape is null, "
	...
	if (
		position.y < 0
		or position.y >= _items.size()
		or not _items[position.y] is Array
		or _items[position.y].is_empty()
		or position.x < 0
	...
	if use_item_size:
		if (
			position.x + item.grid_shape.grid_width > grid_shape.grid_width
			or position.y + item.grid_shape.grid_depth > grid_shape.grid_depth
		):
			printerr("InventoryGrid: uses item size and item does not fit!")
			return false

		for item_y in range(item.grid_shape.grid_depth):
			for item_x in range(item.grid_shape.grid_width):
				if item.grid_shape.get_inventory_cell(item_x, item_y):
					var grid_x := position.x + item_x
					var grid_y := position.y + item_y

					if (
						grid_x >= grid_shape.grid_width
						or grid_y >= grid_shape.grid_depth
					):
						return false

					if not grid_shape.get_inventory_cell(grid_x, grid_y):
						return false
		...
		if (
			position.x >= grid_shape.grid_width
			or position.y >= grid_shape.grid_depth
		):
			return false

		if not grid_shape.get_inventory_cell(position.x, position.y):
			return false
...
func _place_item_at(item: ItemData, position: Vector2i) -> void:
	if item == null:
		return
	...
	if (
		...
		push_error(
			"InventoryGrid: _items array not properly initialized or out "
			+ "of bounds for position %s in _place_item_at. "
			+ "Reinitializing." % str(position)
		)
		initialize()
		return
		...
		for item_y in range(item.grid_shape.grid_depth):
			for item_x in range(item.grid_shape.grid_width):
				if item.grid_shape.get_inventory_cell(item_x, item_y):
					...
					if (
						grid_y >= 0
						and grid_y < grid_shape.grid_depth
						and grid_x >= 0
						and grid_x < grid_shape.grid_width
					):
						_items[grid_y][grid_x] = item
	else:
		if (
			position.y >= 0
			and position.y < grid_shape.grid_depth
			and position.x >= 0
			and position.x < grid_shape.grid_width
		):
...
func try_add_item(item_to_add: ItemData) -> bool:
...
func remove_item(item_to_remove: ItemData) -> ItemData:
...
static func try_transfer_item(
	from_grid: InventoryGrid,
	to_grid: InventoryGrid,
	item_to_transfer: ItemData
) -> bool:
	if item_to_transfer == null:
		print("item given as null")
		return false
	...
	if not to_grid.can_place_item(item_to_transfer):
		print("to inventory cannot place item")
		return false
...
static func try_transfer_item_at(
	from_grid: InventoryGrid,
	to_grid: InventoryGrid,
	item_to_transfer: ItemData,
	coords: Vector2i
	...
	if (
		...
		or not from_grid.has_item(item_to_transfer)
		or not to_grid.can_place_item_at(item_to_transfer, coords)
	):
		return false
...
func can_place_item(item_to_check: ItemData) -> bool:
...
func try_place_item_at(item: ItemData, position: Vector2i) -> bool:
...
func has_item_at(position: Vector2i) -> ItemData:
	if (
		_items.is_empty()
		or position.y < 0
		or position.y >= _items.size()
...
func try_get_item_array() -> Array[ItemData]:
...
func save_data() -> Dictionary:
		...
		items_list.append(
			{
				"name": item.item_name,
				"coords": {
					"x": item.current_invenrtory_coords.x,
					"y": item.current_invenrtory_coords.y
				}
			}
		)
	...
	var ret_dict: Dictionary = {
		...
		"items": items_list,
		"grid_shape": grid_shape.save_data() if grid_shape != null else {},
