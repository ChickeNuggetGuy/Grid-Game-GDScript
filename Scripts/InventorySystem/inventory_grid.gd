		...
		if data.has("items"):
			...
			if items_value is Array: # New, robust format
				for item_data in items_value:
					...
					var result = GameManager.managers["InventoryManager"].try_get_inventory_item(item_name)
					if result["success"]:
				...
				for item_name in items_value:
					var result = GameManager.manager["InventoryManager"].try_get_inventory_item(item_name)
