...
@export var orbit_min_pitch_deg: float = -60.0
@export var orbit_max_pitch_deg: float = -10.0
...
func _transform_rotation(delta: float) -> void:
	...
	if yaw_dir != 0.0:
		orbit_yaw_deg = wrapf(
			orbit_yaw_deg + yaw_dir * orbit_yaw_speed_deg * delta,
			0.0,
			360.0
		)

	if Input.is_action_just_pressed("Camera_Tilt_Up"):
		orbit_pitch_deg -= tilt_step_deg
...
func switch_active_camera(camera_key,unit : Unit, target_position : Vector3):
	...
	if camera_key == "main" :
		...
		phantom_cameras[camera_key].look_at_target = null
	else:
